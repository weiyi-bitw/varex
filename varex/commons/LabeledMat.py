...
class LabeledMat(object):
    ...
    @staticmethod
    def getInteraction(X, verbose=False, log=sys.stderr):
        interacter = PolynomialFeatures(degree=2, interaction_only=True, include_bias=False)
        X_ = interacter.fit_transform(X.data)
        newcolnames = copy.deepcopy(X.colnames)
        for i in range(X.ncol):
            for j in range(i+1, X.ncol):
                newcolnames += [X.colnames[i] + 'X' + X.colnames[j]]
        
        return LabeledMat(X=X_, rownames=X.rownames, colnames=newcolnames, verbose=verbose, log=log)
    ...
    def write2file(self, filename, sep='\t', eol='\n', top_left="#HEADER"):
        fo = open(filename, 'w')
        fo.write(top_left + sep + sep.join(self.colnames) + eol)
        rcnt = 0
        for l in self.data:
